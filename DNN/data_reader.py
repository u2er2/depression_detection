...
def create_vocab(tweets, vocab_size=0):
    ...
    for i in range(len(tweets)):
        ...
        content = chinese_tokenize(tweet)
        for word in content:
            try:
                word_freqs[word] += 1
            except KeyError:
...
def read_dataset(args, vocab_path, MAX_SEQUENCE_LENGTH):
    ...
    # << if statement included with commit eaf8fcb>>
    ...
    if args.humor_data_path:
        ...
        df_humor['sequences_char_unorganized'] = df_humor.tweet.apply(lambda x : strToIndexs2(x,alphabet_dict))
    ...
    if args.sarcasm_data_path:
        ...
        df_sarcasm['sequences_char_unorganized'] = df_sarcasm.tweet.apply(lambda x : strToIndexs2(x,alphabet_dict))
    ...
    # << fix matrix dimensionality mismatch >>
    y_test = turn2(y_test)
    ...
    return X_train_data, X_test_data, y_train, y_test, np.array(train_chars), np.array(test_chars), task_idx_train, task_idx_test, np.array(ruling_embedding_train, dtype=np.float64), \
        np.array(ruling_embedding_test, dtype=np.float64), category_embedding_train, category_embedding_test, vocab

def get_data(args):
    #data_path = args.data_path (instead of args they passed data_path to read_dataset, this resulted in an exception)
        X_train_data, X_test_data, y_train, y_test, train_chars, test_chars, task_idx_train, task_idx_test, train_ruling_embedding, test_ruling_embedding, \
                category_embedding_train, category_embedding_test, vocab = read_dataset(args, args.vocab_path, args.maxlen)
